...
class GameState(object):
    ...
    def update_pacman_hit(self, state):
        HIT_LIMIT = 2

        x, y = state.get_pacman_position()
        walls = state.get_walls()
        if walls[x][y]:
            state.data.hitWall += 1

        if state.data.hitWall > HIT_LIMIT:
            print("Your Pacman Hit Wall {} Times. You cannot hit more than {} times.".format(state.data.hitWall, HIT_LIMIT))
            print("You Lose")
            exit(0)
...
class ClassicGameRules(object):
    ...
    def process(self, state, game):
        ...
        """
        if state.is_win():
            if state.data.hitWall < 1:
                print("You Lose. Your Pacman Need To Hit The Walls At Least Once Time!")
                exit(0)
            print("Your Pacman Hits Walls {} Times".format(state.data.hitWall))
            self.win(state, game)
        if state.is_lose():
